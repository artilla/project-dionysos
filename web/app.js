...
import { createForestMap } from './forest-map.js';
import { createFacilityViewer } from './facilities.js';
...
function persistFilters(){ history.replaceState(null,'',`${location.pathname}?${queryString()}${state.view==='map'?'&view=map':''}`); }
...
const facilityViewer = createFacilityViewer({ api, document });
...
const forestMap = createForestMap({ root: $('forestMap'), api, onOpen: (id, trigger) => loadDetail(id, true, null, trigger) });
...
function render(){
 ...
 $('cards').hidden=state.view==='map'&&matches.length>0;
 forestMap.render(matches,{active:state.view==='map'&&matches.length>0,summaries:new Map(matches.map(f=>[f.id,state.day?`${dateLabel(state.day)} · ${countText(countsFor(f,dateOf(state.day)))}`:`${Number(state.month.slice(4))}월 · ${waitIncluded()?'예약·대기':'입실'} 가능 ${dayList(f).length}일`]))});
 renderConnection();
}
...
function officialForestUrl(forest){
 const query=new URLSearchParams({hmpgId:'FRIP',menuId:'001004',srchSido:forest.regionId||'',insttId:forest.id,srchMonth:state.month});
 const type={stay:'01',camp:'02'}[state.type];if(type)query.set('upperGoodsClsscCd',type);
 return `https://www.foresttrip.go.kr/rep/or/sssn/monthRsrvtStatus.do?${query}`;
}
function renderDetail(){if(!detailId||!detailData)return;stopPrices();const f=detailData,date=dateOf(detailDay),possible=(f.dates[date]||[]).filter(u=>waitIncluded()||u.bookingState!=='wait'),stats=countsFor(f,date),end=dateObject(date);end.setUTCDate(end.getUTCDate()+state.nights);const names={available:'공실',wait:state.nights===1?'예약 대기':'대기는 1박만',full:'예약 완료',unopened:'미개시',closed:'휴무·공사',priority:'우선예약',lottery:'추첨',past:'지난 날짜',unknown:'확인 필요'};
 ...
 $('detailContent').innerHTML=`<div class="dialog-head"><div><div class="kicker">${esc(f.region)} · ${esc(f.operator)} 자연휴양림</div><h2 id="detailTitle">${esc(f.name)}</h2></div><button class="close-btn" data-close aria-label="시설 상세 닫기">${icon('close',18)}</button></div><div class="dialog-content"><div class="detail-preview"><div class="detail-scene">${scene(Number(f.id.replace(/\D/g,''))%4)}</div><div><p>${stamp(f.observedAt)} 확인${f.stale?' · 이전 확인 결과':''}<br>${f.coverage==='complete'?'선택한 범위 확인':'일부 범위만 확인했습니다.'}</p><div class="tags"><span class="tag">${typeName()} · ${state.guests}명 · ${state.nights}박</span></div><a class="secondary naver-review-entry" data-naver-reviews href="/naver-reviews.html?forestId=${encodeURIComponent(f.id)}">네이버 후기 검색 ${icon('arrow',14)}</a></div></div><div class="detail-layout"><section class="detail-calendar" aria-label="입실 날짜 선택"><div class="calendar-heading"><h3>${Number(state.month.slice(4))}월 입실일</h3></div><div class="weekday" aria-hidden="true">${week.map(d=>`<span>${d}</span>`).join('')}</div><div class="calendar-grid">${calendar([f],detailDay,true)}</div><p class="detail-legend">숫자 = ${state.guests}명 · ${state.nights}박 ${waitIncluded()?'예약·대기 가능한':'가능한'} ${unitName()} 수${waitIncluded()?'<br>파란색 = 대기만 가능':''}</p></section><section aria-label="날짜별 시설 현황"><h3 class="rooms-head">${dateLabel(detailDay)} 입실 · ${possible.length}개 ${waitIncluded()?'예약·대기':'가능'}</h3>${waitIncluded()?`<p class="state-counts"><span class="available-count">예약 가능 ${stats.available}개</span><span class="wait-count">예약 대기 ${stats.wait}개</span></p>`:''}<p class="stay-summary">${Number(state.month.slice(4))}월 ${detailDay}일 → ${end.getUTCMonth()+1}월 ${end.getUTCDate()}일 퇴실</p><div class="room-list">${units.length?units.map(u=>{const match=possible.find(p=>p.id===u.id&&p.type===u.type),available=!!match&&match.bookingState!=='wait',inventory=u.days[date]?.state||'unknown';return `<div class="room"><div><div class="room-name">${esc(u.name)}</div><div class="room-cap">${u.capacity===null?'정원 확인 필요':'최대 '+u.capacity+'명'} · ${u.type==='stay'?'숙소':'야영장'}${u.maxNights>0?' · 최대 '+u.maxNights+'박':''}</div></div><span class="room-status" data-state="${available?'available':inventory==='wait'&&state.nights===1?'wait':'unknown'}">${available?'예약 가능':inventory==='available'?'조건 확인 필요':names[inventory]}</span><button type="button" class="room-facility" data-facility-unit="${esc(u.id)}" data-facility-type="${esc(u.type)}">시설 정보·배치도 ${icon('arrow',14)}</button><div class="room-price" ${['available','wait','full'].includes(inventory)?`data-price-unit="${esc(u.id)}" data-price-type="${esc(u.type)}"`:''}>${['available','wait','full'].includes(inventory)?'<span class="price-pending">요금 확인 중…</span>':'<span class="price-unavailable">요금은 숲나들e에서 확인해주세요.</span>'}</div></div>`;}).join(''):'<div class="room-empty">조건에 맞는 시설 정보가 없습니다. 다른 조건으로 조회해주세요.</div>'}</div></section></div><div class="detail-notice">${waitIncluded()?'예약 가능은 공실이 있는 시설, 예약 대기는 취소 자리를 기다리는 시설입니다. 대기 신청은 예약 확정이 아니며 1박 기준입니다.':'선택한 인원과 모든 숙박일의 공실을 확인한 결과입니다.'} 요금은 선택한 숙박기간의 시설 기본요금이며, 추가인원·옵션·감면에 따라 최종 결제금액이 달라질 수 있습니다. 별도 예약 자격·이용 조건과 최신 상태는 숲나들e에서 최종 확인해주세요. 선택한 휴양림과 월의 조회 화면으로 이동합니다.${state.type==='all'?' 숙소 또는 야영장을 선택해주세요.':''} 입실일·인원·숙박일수는 공식 사이트에서 다시 선택해주세요. 로그인이 필요할 수 있습니다.</div><button class="secondary refresh-detail" data-refresh-forest ${running?'disabled':''}>이 휴양림 다시 조회</button></div><div class="dialog-actions"><button class="secondary" data-save="${esc(f.id)}" aria-pressed="${saved.has(f.id)}">${icon('heart',16)} ${saved.has(f.id)?'찜한 숲':'이 숲 찜하기'}</button><a class="primary" href="${esc(officialForestUrl(f))}" target="_blank" rel="noopener noreferrer">숲나들e에서 확인 ${icon('external',15)}</a></div>`;
...
for (const [id, view] of [['listView','list'],['mapView','map']]) $(id).addEventListener('click',()=>{state.view=view;persistFilters();render();});
...
$('detail').addEventListener('close',()=>{facilityViewer.close();stopPrices();detailId=null;detailData=null;if(returnFocus?.isConnected)returnFocus.focus({preventScroll:true});else { const trigger=$('cards').querySelector(`[data-open="${CSS.escape(returnFocus?.dataset?.open||'')}"]`);if(trigger)trigger.focus({preventScroll:true});else $('savedNav').focus({preventScroll:true}); }});
document.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;
 ...
 else if(b.dataset.facilityUnit&&detailData){const unit=detailData.units.find(u=>u.id===b.dataset.facilityUnit&&u.type===b.dataset.facilityType);if(unit)facilityViewer.open({forestId:detailData.id,unitId:unit.id,type:unit.type,title:unit.name,forestName:detailData.name,trigger:b});}
